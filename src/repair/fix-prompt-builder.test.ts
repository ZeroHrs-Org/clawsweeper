...
test("fix prompt includes ZeroHrs mobile feedback proof expectations", () => {
  ...
  assert.match(prompt, /real proof run/);
  assert.match(prompt, /CRABBOX_SSH_KEY/);
  assert.match(prompt, /CRABBOX_STATIC_HOST/);
  assert.match(prompt, /do not run `scripts\/crabbox\/bootstrap-hetzner-android-runner\.sh`/);
  ...
  assert.match(prompt, /without fabricating media/);
