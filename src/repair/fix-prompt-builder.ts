...
function renderTargetSpecificRepairGuidance(fixArtifact: LooseRecord) {
  ...
  return [
    ...
    "- `before-*` media must show the current-main reproduction before the fix; `after-*` media must show the same issue path on this fixed branch after the fix; do not rename same-checkout early/later screenshots into before/after proof; launcher or generic start-page screenshots alone are invalid;",
