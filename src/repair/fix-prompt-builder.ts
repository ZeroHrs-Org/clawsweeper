...
function renderTargetSpecificRepairGuidance(fixArtifact: LooseRecord) {
  ...
  return [
    ...
    "- when a billing, onboarding, entitlement, referral, or account gate blocks the issue route, create and use local admin/referral codes, activate local trial/plan state, or seed minimal local account data as needed to reach the actual in-app screen; document the commands, code type, and seeded state in `command.log` and `proof-manifest.json`, and never use production data;",
    "- if the app opens on the billing gate, use the visible real UI entrypoint such as `Compare Plans`, or create/use local admin or referral codes to unlock the reported route; do not keep retrying an Account-tab coordinate path when the UI dump shows billing-gate text;",
    "- missing local `adb`, `emulator`, or `ffmpeg` on the GitHub Actions runner is not a proof blocker; Android proof must run on the configured Crabbox SSH runner, using `bash scripts/crabbox/run-android-proof.sh` for the real proof run or `crabbox run --provider ssh ...` for issue-specific remote commands;",
    "- in GitHub Actions, `CRABBOX_SSH_KEY`, `CRABBOX_STATIC_HOST`, and SSH known_hosts are prepared by the workflow; do not run `scripts/crabbox/bootstrap-hetzner-android-runner.sh` inside an issue implementation unless the fix artifact explicitly asks you to repair runner provisioning;",
    "- do not edit shared proof harness files for one issue: `scripts/crabbox/android-proof.sh`, `scripts/crabbox/run-android-proof.sh`, `scripts/crabbox/bootstrap-hetzner-android-runner.sh`, their tests, and `docs/crabbox-hetzner-feedback.md` are protected infrastructure and ClawSweeper restores them before staging; issue-specific navigation belongs in a temporary proof driver, inline Crabbox command, or files under `reports/clawsweeper/android-proof`, not in shared scripts;",
    "- navigate through the real app UI using adb taps/swipes/text, emulator controls, or automation in an external temporary proof driver; do not alter the app navigation tree, screens, env config, or tests to expose a proof-only route;",
    "- forbidden in the target PR diff: proof route files, `zeroHrsProofRoute`, `proofRoute`, `EXPO_PUBLIC_*PROOF*`, `Constants.expoConfig` proof plumbing, and tests whose only purpose is proof navigation;",
    "- temporary proof helpers may exist outside the committed target app diff or under ignored `reports/clawsweeper/android-proof` artifacts only;",
    "- do not set `ZEROHRS_ANDROID_PROOF_DRY_RUN=1`, do not pass `--dry-run`, and do not create placeholder or fake png/mp4 media; dry-run output is diagnostic only and invalid issue proof;",
    '- do not return saying Android proof still needs to be generated; run the configured Crabbox SSH proof path before returning, copy the resulting files into `reports/clawsweeper/android-proof`, and verify the directory with `find reports/clawsweeper/android-proof -maxdepth 1 -type f`; if the live proof cannot complete because of SSH host-key, auth, runner, or other external infrastructure failure, write `proof-manifest.json` with `status: "blocked"` plus `command.log` and any real logs that were produced, then stop without fabricating media;',
    ...
    "- `before-*` media must show the current-main reproduction before the fix; `after-*` media must show the same issue path on this fixed branch after the fix; the manifest `after_ref`/`captures.after.ref` must identify the fixed branch/head, never `main`/`origin/main`; do not rename same-checkout early/later screenshots into before/after proof; launcher or generic start-page screenshots alone are invalid;",
    "- if Expo Dev Launcher is visible, open the configured development server and navigate into the app route before recording; if the run cannot leave the launcher, set the manifest `status` to `blocked` with the concrete blocker instead of `completed`; include `launcher_screen_detected: false` on both before and after captures only after verifying the app route is visible;",
    "- the proof manifest must state the issue-specific reproduction route, distinct refs/SHAs or branch names used for before and after, seed/mock commands, and any blocker; set `status` to `completed` only when both before and after media are valid;",
    "- completed proof manifests must include `captures.before.issue_reproduced: true`, a non-empty `captures.before.issue_evidence` string describing the visible pre-fix bug, `captures.after.issue_resolved: true`, and a non-empty `captures.after.fix_evidence` string describing the visible fixed behavior;",
    "- after `scripts/crabbox/run-android-proof.sh` or a direct Crabbox command returns, inspect `reports/clawsweeper/android-proof/proof-manifest.json`; if the base script did not write `issue_evidence` and `fix_evidence`, update the manifest with the specific visible before/after observations from the captured media before returning;",
    "- when reproducing feedback needs account, app, billing, referral, entitlement, or database state, seed or mock minimal local dev/test rows and document the seed; do not use production data;",
    "- keep the Codex edit pass narrow and proof-first: after the code change, run focused changed-surface tests, then capture Android proof; do not run `pnpm run dev:mobile:local`, broad `pnpm run diff:checks`, or long-lived stack startup inside the edit pass unless the target code change directly touches that stack, because ClawSweeper runs configured validation after checkpointing the patch;",
