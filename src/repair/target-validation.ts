...
const ZEROHRS_TARGET_REPO = "zerohrs-org/zerohrs-app";
const ZEROHRS_TARGET_ENV_B64 = "ZEROHRS_TARGET_ENV_B64";
const ZEROHRS_VALIDATION_ENV_DEFAULTS: Record<string, string> = {
  AUTH_AUTO_ALLOCATE_INSTANCE: "false",
  COMPOSIO_API_KEY: "clawsweeper-validation-composio-key",
  COMPOSIO_LOCAL_TRIGGER_SUBSCRIPTION_ENABLED: "false",
  COMPOSIO_WEBHOOK_SECRET: "clawsweeper-validation-composio-webhook-secret",
  DATABASE_URL: "postgres://postgres:postgres@127.0.0.1:5432/zerohrs_clawsweeper_validation",
  DEPLOYMENT_ENV: "docker",
  DEV_MODE: "false",
  DISABLE_ONBOARDING_AGENTS: "true",
  ENABLE_BACKGROUND_WORKERS: "false",
  GOOGLE_ANDROID_CLIENT_ID: "zerohrs-validation-android-client-id",
  GOOGLE_IOS_CLIENT_ID: "zerohrs-validation-ios-client-id",
  ...
  RESEND_API_KEY: "clawsweeper-validation-resend-key",
...
export function prepareTargetToolchain(cwd: string, options: TargetValidationOptions) {
  ...
  const validationEnv = targetValidationEnv(options);
...
export function prepareTargetValidationEnvironment(cwd: string, options: TargetValidationOptions) {
  if (!isZeroHrsTargetRepo(options.targetRepo)) return;
  const body = zeroHrsTargetEnvBody();
  writeZeroHrsTargetEnvIfMissing(path.join(cwd, ".env"), body);
  const controlPlaneDir = path.join(cwd, "backend", "control-plane");
  if (fs.existsSync(controlPlaneDir)) {
    writeZeroHrsTargetEnvIfMissing(path.join(controlPlaneDir, ".env"), body);
  }
}

function writeZeroHrsTargetEnvIfMissing(envPath: string, body: string) {
  if (fs.existsSync(envPath)) return;
  fs.mkdirSync(path.dirname(envPath), { recursive: true });
  fs.writeFileSync(envPath, body, { mode: 0o600 });
}

function zeroHrsTargetEnvBody() {
  const encoded = process.env[ZEROHRS_TARGET_ENV_B64]?.trim();
  if (encoded) {
    const decoded = Buffer.from(encoded, "base64").toString("utf8");
    if (!decoded.trim() || !/^[A-Za-z_][A-Za-z0-9_]*=/m.test(decoded) || decoded.includes("\0")) {
      throw new Error(`${ZEROHRS_TARGET_ENV_B64} is not a valid dotenv payload`);
    }
    return decoded.endsWith("\n") ? decoded : `${decoded}\n`;
  }
  return (
    [
      "# Generated by ClawSweeper for validation-only ZeroHrs CI runs.",
      "# Values are non-secret local placeholders; production credentials are not required.",
      ...Object.entries(ZEROHRS_VALIDATION_ENV_DEFAULTS).map(([key, value]) => `${key}=${value}`),
    ].join("\n") + "\n"
  );
}

function targetValidationEnv(options?: Pick<TargetValidationOptions, "targetRepo">) {
  const env: NodeJS.ProcessEnv = {
    ...process.env,
    CI: process.env.CI ?? "true",
    OPENCLAW_LOCAL_CHECK: process.env.OPENCLAW_LOCAL_CHECK ?? "0",
  };
  if (options && isZeroHrsTargetRepo(options.targetRepo)) {
    for (const [key, value] of Object.entries(ZEROHRS_VALIDATION_ENV_DEFAULTS)) {
      env[key] = env[key] || value;
    }
    for (const [key, value] of parseDotenvBody(zeroHrsTargetEnvBody())) {
      env[key] = env[key] || value;
    }
  }
  ...
  delete env[ZEROHRS_TARGET_ENV_B64];
  return env;
}

function parseDotenvBody(body: string) {
...
function isZeroHrsTargetRepo(repo: string) {
  return repo.toLowerCase() === ZEROHRS_TARGET_REPO;
}
